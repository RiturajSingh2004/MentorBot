    ...
    <div className={cn("flex flex-col", isUser ? "items-end" : "items-start")}>
      ...
      <div className={cn(chatBubbleVariants({ isUser, animation }), className)}>
        ...
        {role === "assistant" && actions ? (
          <div className="absolute -bottom-4 right-2 flex space-x-1 rounded-lg border bg-background p-1 text-foreground opacity-0 transition-opacity group-hover/message:opacity-80">
            {actions}
          </div>
        ) : null}
      </div>
      ...
      {showTimeStamp && createdAt ? (
        <time
          ...
          className={cn(
            "mt-1 block px-1 text-xs text-white opacity-50 ",
